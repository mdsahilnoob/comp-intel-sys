...
export const explorerQuerySchema = z
  .object({
    ...
    role: optionalText,
    level: z.enum(["ENTRY", "MID", "SENIOR", "STAFF", "PRINCIPAL", "DISTINGUISHED"]).optional(),
  ...
  .superRefine((value, context) => {
    if (value.minTc !== undefined && value.maxTc !== undefined && value.minTc > value.maxTc) {
      context.addIssue({
        code: "custom",
        path: ["maxTc"],
...
export function parseExplorerQuery(values: URLSearchParams | Record<string, string | undefined>): ExplorerFilters {
  const input = values instanceof URLSearchParams ? Object.fromEntries(values.entries()) : values
...
export const submissionSchema = z
  .object({
    ...
    stockAnnual: z.number().finite().int().min(0).max(MAX_COMPENSATION).nullish(),
    bonusAnnual: z.number().finite().int().min(0).max(MAX_COMPENSATION).nullish(),
  ...
  .superRefine((value, context) => {
    const total = value.baseSalary + (value.stockAnnual ?? 0) + (value.bonusAnnual ?? 0)
    if (total > MAX_COMPENSATION) {
      context.addIssue({
        code: "custom",
        path: ["baseSalary"],
        message: "Total compensation exceeds the demo sanity limit of ₹100 crore",
      })
    }
  })
